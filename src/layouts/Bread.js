...
function Bread({ location }) {
  const pathNames = [];
  location.pathname.slice(1).split('/').forEach((item, index) => {
    if (index > 0) {
      pathNames.push(_.camelCase(`${pathNames[index - 1]}-${item}`));
    } else {
  ...
  const breads = breadsArray.map((item, index) => {
    const content = (
    ...
    return (
      <Breadcrumb.Item key={item}>
        {((breadsArray.length - 1) !== index && pathSet[item].clickable)
